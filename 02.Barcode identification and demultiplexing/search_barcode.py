...
def process_file(fastq_file, excel_file, output_folder, fixed_seq1, fixed_seq2, reverse, summary_queue):
    with tempfile.TemporaryDirectory() as temp_output_folder:
        ...
        unique_filename = f"{os.path.basename(fastq_file)}_{pid}_{timestamp}.fastq"


        with open(os.path.join(temp_output_folder, unique_filename), "w") as output_file:
            for record in SeqIO.parse(fastq_file, "fastq"):
                ...
                if index1 != -1 and index2 != -1 and index1 < index2:
                    ...
                    fuzzy_match_barcode1 = fuzzy_match(barcode1, col1)
                    fuzzy_match_barcode2 = fuzzy_match(barcode2, col2)
                    fuzzy_match_barcode3 = fuzzy_match(barcode3, col3)
        ...
        for unique_filename in os.listdir(temp_output_folder):
                shutil.move(os.path.join(temp_output_folder, unique_filename), os.path.join(output_folder, unique_filename))

        summary_queue.put(counts)
...
def count_sequences(directory, excel_file, output_folder, fixed_seq1, fixed_seq2, reverse):
    ...
    for fastq_file in fastq_files:
        pool.apply_async(process_file, args=(fastq_file, excel_file, output_folder, fixed_seq1, fixed_seq2, reverse, summary_queue))
